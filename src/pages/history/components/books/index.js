...
function Books() {
  ...
  React.useEffect(() => {
    document.title = "eWorkbook - History Exercise - Books"
    setSearchParams(formData)
    const getAllMaterials = async () => {
      try {
        const myResponse = await recordApi.getRecordBooksPaging({
          ...formData,
  ...
  return (
    <div className="history-container box tw-box tw-text-center">
      <TopPart current="Books" onNameSearchChange={onNameSearchChange} />
      <div className="tw-hidden md:tw-block header-row-container tw-mb-2">
        <Row className="header-row md:tw-text-lg tw-text-sm tw-font-semibold tw-text-black">
          ...
          <Col span={4} className="tw-pl-1 tw-text-left">{t('history.progress')}</Col>
